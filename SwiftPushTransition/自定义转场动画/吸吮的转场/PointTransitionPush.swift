...
import UIKit
/// 从一个点扩大动画
class PointTransitionPush: NSObject, UIViewControllerAnimatedTransitioning {
    /// 开始动画的frame
...
//        let maskStartBP = UIBezierPath.init(ovalIn: startFrame)
        ...
        if let fromVC = transitionContext.viewController(forKey: .from), let toVC = transitionContext.viewController(forKey: .to) {
            ...
            // 中间过渡的UIBezierPath
            let maskMiddleBP = UIBezierPath.init(ovalIn: startFrame.insetBy(dx: -startFrame.width/2.0, dy: -startFrame.width/2.0))
            // 最终的UIBezierPath
            let maskFinalBP = UIBezierPath.init(ovalIn: startFrame.insetBy(dx: -finalRadius, dy: -finalRadius))
            // 创建一个CAShapeLayer负责展示圆形的遮盖
            let maskLayer = CAShapeLayer()
            maskLayer.path = maskFinalBP.cgPath // 将它最为最终的path，避免动画完成后回弹
            ...
            animation1.toValue = maskMiddleBP.cgPath
            animation1.duration = self.transitionDuration(using: transitionContext)/3.0
            ...
            animation2.fromValue = maskMiddleBP.cgPath
            animation2.toValue = maskFinalBP.cgPath
